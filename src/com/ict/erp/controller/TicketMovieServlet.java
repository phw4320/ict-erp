...
public class TicketMovieServlet extends HttpServlet {
	...
	private static final String UP_PATH = "C:\\jsp_study\\workspace\\ict_erp\\WebContent";
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			} else if (cmd.equals("ticketMovieView")) {
				String tmNum = req.getParameter("tmNum");
				System.out.println("나는 파라미터로 넘어온 넘버: "+tmNum);
				TicketMovie tm = new TicketMovie();
				tm.setTmNum(Integer.parseInt(tmNum));
				req.setAttribute("tm", ts.selectTicketMovie(tm));
			}
			...
			if(cmd.equals("ticketMovieInsert")) {
				...
				for(FileItem fi:fList) {
					if(fi.isFormField()) {
						...
						System.out.println(fi.getFieldName()+" = "+fName);
					}
				}
				...
				TicketMovie tm = new TicketMovie();
				tm.setTmNum(Integer.parseInt(tmNum));
				req.setAttribute("tm", ts.deleteTicketMovie(tm));
				DiskFileItemFactory dfFactory = new DiskFileItemFactory(THRESHOLD_SIZE, TEMP_REPOSITORY);
