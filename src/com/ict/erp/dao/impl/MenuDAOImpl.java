...
public class MenuDAOImpl extends CommonDAOImpl implements MenuDAO{
	...
	@Override
	public List<MenuInfo> selectMenuNonePageList(MenuInfo mei) throws SQLException {
		List<MenuInfo> meiList = new ArrayList<MenuInfo>();
		String sql = "select * from menu_info";
		try {
			ps = con.prepareStatement(sql);
			rs = ps.executeQuery();
			meiList = ICTUtils.parseList(rs, MenuInfo.class);
		}catch(SQLException e) {
			throw e;
		}finally {
			close();
